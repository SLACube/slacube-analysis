import h5py
import numpy as np
from slacube.utils import match_unix_timestamp

def get_n_blocks(fpath, buf_size):
...
def build_event(fpath, buf_size, window=1820//2):
    with h5py.File(fpath, 'r') as f:
        ...
        for i_blk in range(n_blocks):
            ...
            mask = block['packet_type'] == 4
            
            if not np.any(mask):
                continue
                
            first_unix_ts = np.array(block[mask][0])
            break
        
        if first_unix_ts is None:
        ...
        for i_blk in range(n_blocks):
            ...
            yield builder.build_events(data_buf, unix_ts)
